...
select count(distinct ID_Sejour) as Sejour
,count(distinct no_ach_conf) as Nb_reservations , 
...
and ID_JOUR_DEBUT_SEJOUR between 20200914 and 20210108
and ID_JOUR_RESERVATION between 20200908 and 20200922
...
where LB_campagne = '20200908_Ventes exclusives Automne'
...
select sum( isnull([MT_OFFRE_ACT_ID],0) + isnull([MT_SUPPLEMENTS],0)			
+ isnull([MT_OPTIONS],0) + isnull([MT_SERVICES],0)) from #perimetres where hb_com=1 
...
sum( case when hb_com = 1 then isnull([MT_OFFRE_ACT_ID],0) + isnull([MT_SUPPLEMENTS],0)			
+ isnull([MT_OPTIONS],0) + isnull([MT_SERVICES],0) else 0 end ) as Ca_HB_Com_ttc
from #perimetres 
