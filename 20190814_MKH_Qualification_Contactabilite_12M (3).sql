...
Select --'20180501' 
DATEADD(year,-1,DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0)) as Dt_Min, -- 1er jour mois en cours -1 an 
--'20190430' 
DATEADD(day, -1, DATEADD(month, DATEDIFF(month, 0, GETDATE()), 0)) as Dt_Max -- dernier jour mois précédent  
into  #perimetre


 drop table #contact
select s.[Code_Etablissement], s.masterId,
...
from
  ...
  FROM [BARRIERE].[dbo].[H_Sejour] 
   where [Code_Etablissement] <> 'MARNA' --- like (select hotel from #perimetre) 
  and [Date_Debut_du_Sejour] between (select dt_min from #perimetre)  and (select dt_max from #perimetre) 
 ...
 left join 
  ...
  where AdressePrincipale = 1 and ContenuID = 1 group by  MasterID) x
...
left join
  ...
  where [TelephonePrincipal] = 1 and ContenuID = 1 group by  MasterID) x
...
left join
  ...
  where ContenuID = 1 and (AdrLigne1 is not null or AdrLigne4 is not null) group by  MasterID) x
